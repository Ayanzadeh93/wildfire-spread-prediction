...
def make_dataset_from_config(
  ...
  """Convenience wrapper around get_dataset for direct use.

  Args:
    file_pattern: Glob pattern for TFRecord files.
    data_size: Tile size as stored in TFRecords (64).
    sample_size: Tile size fed into the model (64).
    batch_size: Mini-batch size.
    input_features: Tuple of feature names.
    output_features: Tuple of output feature names.
    ...
    compression_type: TFRecord compression ('' for none).
    shuffle_buffer_size: Buffer size for shuffling.

  Returns:
    A tf.data.Dataset.
  """
  _validate_input_features(list(input_features))
  _validate_output_features(list(output_features))
  return get_dataset(
