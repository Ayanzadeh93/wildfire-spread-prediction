...
def encoder(
  ...
  for i in range(len(layers_list[1:])):
    ...
    net = model_utils.res_block(
        ...
        l1_regularization=l1_regularization,
        l2_regularization=l2_regularization)
    print(f'Encoder layer {i} output shape:', net.shape)
  return net
...
def decoder(
  ...
  """
  net = input_tensor
  print('Decoder input shape:', net.shape)
  for i, (layer, pool) in enumerate(zip(layers_list, pool_list)):
    net = decoder_block(net, layer, pool, dropout, batch_norm,
                        l1_regularization, l2_regularization)
    print(f'Decoder layer {i} output shape:', net.shape)
  return net
