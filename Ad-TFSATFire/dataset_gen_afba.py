...
if __name__ == '__main__':
    ...
    data_path = os.environ.get('TS_SATFIRE_DATA', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ts-satfire'))
    if modes == 'train' or modes == 'val':
        satimg_processor.dataset_generator_seqtoseq(mode=modes, usecase=usecase, data_path=data_path, locations=locations, visualize=False, 
        ...
        for id in locations:
            root_path = os.environ.get('TS_SATFIRE_ROOT', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dataset'))
            if usecase == 'ba':
                satimg_processor.dataset_generator_seqtoseq(mode = 'test', usecase=usecase, data_path=data_path, locations=[id], visualize=False, file_name=usecase+'_'+id+'_img_seqtoseql_'+str(ts_length)+'i_'+str(interval)+'.npy', label_name=usecase+'_'+id+'_label_seqtoseql_'+str(ts_length)+'i_'+str(interval)+'.npy',
                ...
                af_test_processor.af_seq_tokenizing_and_test_slicing(location=id, modes=modes, ts_length=ts_length, interval=interval, usecase=usecase, root_path=root_path, save_path=root_path)