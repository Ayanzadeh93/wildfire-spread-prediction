        ...
        for location in locations:
            ...
            for i in range(0, file_list_size, interval):
                if i + ts_length >= file_list_size:
                    i = max(file_list_size - ts_length, 0)
...
class AFTestDatasetProcessor(SatProcessingUtils):  
    ...
    def af_seq_tokenizing_and_test_slicing(self, location, modes, ts_length, interval, usecase, root_path, save_path):
        ...
        if location in ['val', 'train']:
            ...
            import os
            data_path = os.environ.get('TS_SATFIRE_DATA', os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'ts-satfire'))
            root_path = os.path.join(data_path, 'data_train_proj2') if os.path.exists(os.path.join(data_path, 'data_train_proj2')) else data_path
            save_path = os.environ.get('TS_SATFIRE_ROOT', os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'dataset'))
